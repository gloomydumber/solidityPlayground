...
// lecture 35 - fallback / receive 함수 (0.6v 이전 fallback)
...
contract Bank{
  ...
  // ~0.6 
  function() external payable { // fallback 함수
    emit JustFallbackWIthFunds(msg.sender, msg.value, "JustFallbackWIthFunds is called"); // 함수내용 = 이더를 밥고나서 취하는 액션 (여기선 이벤트 발생)
  }
}
...
contract You{
  ...
  function JustGiveMessage(address _to) public{
    // ~ 0.7
    (bool sent, ) = _to.call("HI"); // call은 이더만 보내는 것이 아니라, 함수를 실행 시키는 기능도 있음 (여기서는 Bank Contract에 "HI"같은 함수는 없기 때문에, fallback으로 진행)
    require(sent,"Failed to send either");
